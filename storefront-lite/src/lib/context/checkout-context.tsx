...
interface CheckoutContext {
  ...
  setShippingOption: (soId: string, data?: any) => void
...
export const CheckoutProvider = ({ children }: CheckoutProviderProps) => {
  ...
  const { mutateAsync: updateCart, isLoading: updatingCart } = useUpdateCart(
    cart?.id!
  )
  const updateCustomCart = useMutation<StoreCartsRes, Error, any>({
    mutationFn: async (data) =>
      await medusaClient.client.request(
        "POST",
        `/store/custom-cart/${cart?.id}`,
        data
      ),
  })
   ...
   */
  const setShippingOption = (soId: string, data?: any) => {
    if (cart) {
      setShippingMethod(
        { option_id: soId, data },
        {
  ...
  const setAddresses = async (data: CheckoutFormValues) => {
    ...
    const payload: StorePostCartsCartReq & { deliver_at: string } = {
      shipping_address,
      email,
      deliver_at: data.deliver_at.toISOString(),
    }
    ...
    // }
    payload.billing_address = billing_address
    payload.billing_address.first_name = shipping_address.first_name
    payload.billing_address.last_name = shipping_address.last_name
    ...
    await updateCustomCart.mutateAsync(
      {
        deliver_at: data.deliver_at.toISOString(),
      },
      {
...
const mapFormValues = (
  ...
  return {
    ...
    deliver_at: (cart as any)?.deliver_at
      ? new Date((cart as any).deliver_at)
      : new Date(),
  }
}
