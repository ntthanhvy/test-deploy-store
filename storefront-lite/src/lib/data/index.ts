...
export async function getFeaturedCollections(
  ...
  const { collections, count } = await medusaRequest(
    "GET",
    "/custom/collections",
    {
      query: {
...
export async function getStoresList({
  ...
  response: { stores: StoreLocation[]; count: number }
  nextPage: number
}> {
  const limit = queryParams.limit || 12

  const { stores, count, nextPage } = await medusaRequest(
    "GET",
    "/store-locations",
    {
      query: {
  ...
  return {
    response: { stores, count },
    nextPage,
  }
}

export async function getPageBySlug(slug: string) {
  const data = await medusaRequest("GET", `/pages/${slug}`)
    .then((res) => res.body)
    .catch((err) => {
      throw err
    })
...
export async function getStoreDetail() {
  const data = await medusaRequest("GET", `/store`)
    .then((res) => res.body)
    .catch((err) => {
      throw err
    })
